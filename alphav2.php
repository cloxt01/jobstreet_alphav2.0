...
$url_job = "https://www.jobstreet.co.id/api/chalice-search/v4/search?siteKey=ID-Main&sourcesystem=houston&userqueryid=98fb54df525fed4ec5d86e111a6cc3d3-2245169&userid=$session_id&usersessionid=$session_id&eventCaptureSessionId=$session_id&page=1&seekSelectAllPages=$seekSelectAllPages&classification=$classification&sortmode=$sortmode&pageSize=$pageSize&include=seodata&locale=id-ID";
  ...
  foreach ($graphql_params as $i => $data){
    ...
    if ($i == 0) {
      if(isset($graphql_decode['errors'][0]['extensions']['code'])){
        echo("[token] token telah kadaluwarsa\n");
        die();
      }
      ...
      if (isset($linkOut) && $linkOut == false){
        $questionnaire = $graphql_decode[0]['data']['jobApplicationProcess']['questionnaire'];
